...
const MetricCard = ({ icon: Icon, label, value, isCurrency = false, loading }) => {
    return (
        <motion.div
            className="bg-gradient-to-r from-indigo-50 to-purple-50  backdrop-blur-sm p-6 rounded-xl border border-gray-100 hover:border-purple-100 hover:shadow-lg transition-all"
            whileHover={{ y: -4 }}
        >
            ...
            studSnap.forEach(docSnap => {
                ...
                if (isActive) {
                    ...
                    (s.transactions || []).forEach(tx => {
                        ...
                        const isToday = ts.getFullYear() === today.getFullYear() &&
                        ...
                        if (tx.status === 'completed' && tx.academicYear === s.academicYear) {
