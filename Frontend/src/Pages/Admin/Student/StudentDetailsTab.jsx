...
import {
  Bus, CalendarDays, CreditCardIcon, GraduationCapIcon, Hash,
  HomeIcon, Mail, Phone, Ticket, User, Utensils, VenusAndMars
} from 'lucide-react';
...
function StudentDetailsTab({ formData, setFormData, schoolData }) {
  ...
  useEffect(() => {
    const fetchDestinations = async () => {
      try {
        const snapshot = await getDocs(collection(db, 'destinationList'));
        const destinations = [];
        snapshot.forEach(doc => {
          destinations.push(...doc.data().destinations); // flattening array
        });
  ...
  const handleBusStopChange = (e) => {
    ...
    const selectedDestination = busDestinations.find(
      dest => dest.name === selectedStop && dest.academicYear === formData.academicYear
    );
    setFormData({
      ...formData,
      busStop: selectedStop,
      transportFee: selectedDestination ? selectedDestination.fee : 0,
    });
  };
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* All your sections remain the same */}
        ...
        <div className="md:col-span-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <SelectField
              icon={<Bus className="w-5 h-5" />}
              label="Bus Stop"
              options={busDestinations
                .filter(dest => dest.academicYear === formData.academicYear)
                .map(dest => dest.name)}
              value={formData.busStop}
...
const SelectField = ({ icon, label, options = [], ...props }) => (
  <div className="space-y-2">
    ...
    <select
      {...props}
      className="w-full px-4 py-2.5 border border-gray-200 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none bg-white"
    >
