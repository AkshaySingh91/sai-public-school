...
function StockGroup() {
  ...
  useEffect(() => {
    const allGroups = classOrder.flatMap((cls) => [
  ...
  const fetchItems = async (className, category) => {
    ...
    try {
      // 1) fetch all stocks for this school
      const snap = await getDocs(
        collection(db, "allStocks")
      );
      // 2) filter by schoolCode, class range, and category
      const fetched = snap.docs
        .map((d) => ({ id: d.id, ...d.data() }))
        .filter((stock) => {
          if (stock.schoolCode !== userData.schoolCode) return false;
          const fromIndex = classOrder.indexOf(stock.fromClass);
          const toIndex = classOrder.indexOf(stock.toClass);
          const groupIndex = classOrder.indexOf(className);
          const inRange =
            fromIndex !== -1 && toIndex !== -1 &&
            groupIndex >= fromIndex && groupIndex <= toIndex;
          const catMatch = stock.category === category || stock.category === "All";
                                ...
                                <div className="overflow-x-auto rounded-xl border border-purple-100 shadow-sm">
                                  <table className="min-w-full">
                                    <thead className="bg-gradient-to-r from-purple-100 to-violet-100">
                                      <tr>
                                        {["Item Name", "Quantity", "Unit Price", "Total Value"].map((header, i) => (
                                          <th
                                            key={i}
