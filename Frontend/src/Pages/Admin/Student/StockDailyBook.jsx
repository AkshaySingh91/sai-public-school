...
export default function StockDailyBook() {
            ...
            snap.forEach((studentDoc) => {
                ...
                (studentData.StockPaymentDetail || []).forEach((stockTx) => {
                    ...
                    if (txDate >= fromDate && txDate <= toDate) {
                        // Calculate totals
                        const totalQuantity = stockTx.items.reduce((sum, item) => sum + (Number(item.quantity) || 0), 0);
                        console.log({ totalQuantity })

                        const totalAmount = stockTx.items.reduce((sum, item) => sum + ((Number(item.total) || 0)), 0);

                        const itemsList = stockTx.items.map(item =>
    ...
    const exportPDF = () => {
        const doc = new jsPDF();
        const headers = [
            ["Date", "Student", "Class", "Gender", "Items",
                "Total Qty", "Total Amount", "Account", "Receipt", "Status"]
        ];
                    ...
                    {loading ? (
                        ...
                        <>
                            <div className="flex flex-wrap items-center gap-4 mb-6">
                                {/* Date inputs remain the same but updated focus states */}
                                <div className="flex items-center gap-2">
                                    ...
                                    <input
                                        ...
                                        className="px-3 py-2 border border-purple-200 rounded-lg text-sm focus:ring-2 focus:ring-violet-500 focus:border-transparent bg-purple-50"
                                    />
                                </div>
                                <div className="flex items-center gap-2">
                                ...
                                <button
                                    ...
                                    className="flex items-center px-5 py-2.5 text-sm font-medium text-white bg-gradient-to-r from-purple-500 to-violet-600 rounded-xl hover:from-purple-600 hover:to-violet-700 transition-all shadow-md"
                                >
                                    Show
                                </button>
                            </div>
